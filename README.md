...
> A production-grade RAG chatbot that answers developer questions about the [Mastra AI framework](https://mastra.ai). Built as a freelance portfolio piece: clean code, real evals, deployable to Vercel + Supabase.
...
- Supabase pgvector for storage with an in-memory fallback (seeded with hand-written fixtures) for dev.
...
1. Uses an **in-memory vector store** seeded with hand-written Mastra fixture docs.
...
When `POSTGRES_CONNECTION_STRING` is unset, the app uses the in-memory vector store. To populate it from real Mastra docs:

```bash
npm run ingest -- --source=docs --dry-run   # see the plan
npm run ingest -- --source=docs             # actually fetch + embed + upsert
```

For pgvector in production, set `VECTOR_BACKEND=pg` and point `POSTGRES_CONNECTION_STRING` at a real pgvector instance (Supabase is the documented choice; see [DEPLOY.md](./DEPLOY.md)).
...
| `npm test` | Vitest unit + integration tests. 85 tests, all passing. |
...
2. **Vector store** (`src/lib/vector/`) — the factory defaults to `InMemoryVectorStore`, pre-seeded at construction with a hand-written corpus of 10 Mastra fixture documents (`src/lib/vector/fixtures.ts`). The corpus covers RAG, agents, workflows, storage, deployment, integrations, and source code excerpts. Vectors for fixtures are produced by the same local embedder so retrieval actually works.
